...
model_gb<-function(G_mtr,pheno,y_name,fixed=NULL,random){
  if(!is.null(fixed)){
    ...
    cat("..........................................................", "\n")
    gb<-gblup(rsp = y_name,data = pheno,pos = c(T,T,T),
...
# model_gb<-function(G_mtr,pheno,y_name,fixed=NULL,random =NULL,pos = c(T,T,T)){
#   if(!is.null(fixed)){
#     model_spec<-c(formula(paste("y ~ ", paste(fixed,collapse = " + "))),
#                   (formula(paste("y ~",random))))
#     cat("..........................................................", "\n")
#     gb<-gblup(rsp = y_name,data = pheno,pos,
#               design =  c(formula(paste(" ~ ", paste(fixed,collapse = " + "))),
#                           (formula(paste("~", random)))),G = G_mtr)
#     print(summary(gb, fe=TRUE))}
#   else if(is.null(fixed)){
...
#       print(summary(gb))}
#     else (is.null(random)){
#       model_spec<-c(formula(y~1), (formula(paste("~"))))
#       cat("..........................................................", "\n")
#       gb<-gblup(rsp = y_name,data = pheno,pos,
#               design =  c(formula(y~1), (formula(paste("~")))),G = G_mtr)
#       print(summary(gb))}
#   }
#   cat("..........................................................", "\n")
#   return(gb)
...
cross_validation<- function(G,pheno,iterate=5,y_name,fixed=NULL,rand_var, poly_name=NULL, pop=NULL, seed=1900){
  ...
  for (i in 1:iterate) {
    ...
    if(!is.null(fixed)){
      gb_cross<-gblup(rsp = y_name,data = d_pheno, pos = c(T,T,T),
                      design =  c(formula(paste(" ~ ", paste(fixed,collapse = " + "))),
